...
public class AppEventsLogger
{
    ...
    static class PersistedAppSessionInfo
    {
        ...
        private static FacebookTimeSpentData getTimeSpentData(Context context1, AccessTokenAppIdPair accesstokenappidpair)
        {
            ...
            FacebookTimeSpentData temp = facebooktimespentdata;
            if(facebooktimespentdata == null)
            {
                temp = new FacebookTimeSpentData();
        ...
        static void saveAppSessionInformation(Context context1)
        {
            ...
            try {
                if (!flag) {
                    ...
                    temp = new ObjectOutputStream(new BufferedOutputStream(context1.openFileOutput("AppEventsLogger.persistedsessioninfo", 0)));
                    temp.writeObject(appSessionInfoMap);
                    hasChanges = false;
...
;
    ...
    static class PersistedEvents
    {
        ...
        public static void persistEvents(Context context1, Map map)
        {
            ...
            PersistedEvents temp = readAndClearStore(context1);
            Iterator iterator = map.entrySet().iterator();
            for(;iterator.hasNext();) {
                java.util.Map.Entry entry = (java.util.Map.Entry)iterator.next();
                List list = ((SessionEventsState)entry.getValue()).getEventsToPersist();
                if(list.size() != 0)
                    temp.addEvents((AccessTokenAppIdPair) entry.getKey(), list);
            }
            temp.write();
        }
        ...
        public static PersistedEvents readAndClearStore(Context context1)
        {
            synchronized(staticLock)
            {
                PersistedEvents temp = new PersistedEvents(context1);
                temp.readAndClearStore();
                return temp;
            }
        }
    ...
    private static FlushStatistics buildAndExecuteRequests(FlushReason flushreason, Set set)
    {
        ...
        if(arraylist.size() > 0)
        {
            ...
            for(;iterator2.hasNext();) {
                ((Request)iterator2.next()).executeAndWait();
            }
        } else
        {
            flushreason = null;
    ...
    private static Request buildRequestForSession(AccessTokenAppIdPair accesstokenappidpair, SessionEventsState sessioneventsstate, boolean flag, FlushStatistics flushstatistics)
    {
        ...
        final AccessTokenAppIdPair accessTokenAppId = accesstokenappidpair;
        final Request postRequest = request;
        final SessionEventsState sessionEventsState = sessioneventsstate;
        final FlushStatistics flushState = flushstatistics;
    ...
    {
        final FlushReason reason = flushreason;
    ...
    private static void flushAndWait(FlushReason flushreason)
    {
            synchronized(staticLock)
            {
                if(!requestInFlight) {
                    ...
                    try
                    {
                        FlushStatistics temp = buildAndExecuteRequests(flushreason, hashset);
                        if(flushreason != null)
                        {
                            obj = new Intent("com.facebook.sdk.APP_EVENTS_FLUSHED");
                            ((Intent) (obj)).putExtra("com.facebook.sdk.APP_EVENTS_NUM_EVENTS_FLUSHED", ((FlushStatistics) (temp)).numEvents);
                            ((Intent) (obj)).putExtra("com.facebook.sdk.APP_EVENTS_FLUSH_RESULT", ((FlushStatistics) (temp)).result);
    ...
    private static SessionEventsState getSessionEventsState(AccessTokenAppIdPair accesstokenappidpair)
    {
        synchronized(staticLock)
        {
            SessionEventsState temp = (SessionEventsState)stateMap.get(accesstokenappidpair);
            return temp;
        }
    }
    ...
    private static void handleResponse(AccessTokenAppIdPair accesstokenappidpair, Request request, Response response, SessionEventsState sessioneventsstate, FlushStatistics flushstatistics)
    {
        ...
        if(Settings.isLoggingBehaviorEnabled(LoggingBehavior.APP_EVENTS))
        {
            ...
            try
            {
                temp = (new JSONArray(temp)).toString(2);
            }
            // Misplaced declaration of an exception variable
            catch(Exception ex)
            {
            ...
            Logger.log(LoggingBehavior.APP_EVENTS, TAG, "Flush completed\nParams: %s\n  Result: %s\n  Events JSON: %s", new Object[] {
                request.getGraphObject().toString(), s, temp
            });
        }
        if(facebookrequesterror != null)
            flag = true;
    ...
    private static void logEvent(final Context context1, final AppEvent appevent, final AccessTokenAppIdPair accesstokenappidpair)
    {
        Settings.getExecutor().execute(new Runnable() {

            public void run()
            {
                AppEventsLogger.getSessionEventsState(context1, accesstokenappidpair).addEvent(appevent);
    ...
    {
        AppEvent temp = new AppEvent(context, s, double1, bundle, flag);
        logEvent(context, ((AppEvent) (temp)), accessTokenAppId);
    }
    ...
    private static void setSourceApplication(Activity activity)
    {
        ...
        if(activity == null || intent.getBooleanExtra("_fbSourceApplicationHasBeenSet", false))
        {
            resetSourceApplication();
            return;
        }
    ...
    {
        AccessTokenAppIdPair temp = new AccessTokenAppIdPair(session);
        return accessTokenAppId.equals(temp);
    }
